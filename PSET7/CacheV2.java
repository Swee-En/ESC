...
public class CacheV2 {
	...
	public List<Integer> service (final int input) throws Exception {
		...
		if (f == null) {
			...
			f = ft;
			results.put(input, ft); //telling others he's working on it
			ft.run(); //actually working on it now
		}

		return f.get(); //someone is working on it, will get it done eventually
	}
